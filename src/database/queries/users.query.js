...
export const GET_USER_BY_ID_WITH_ROLES = `SELECT u.id, u.name, u.email, u.image, u.active, ARRAY_AGG(
                                            json_build_object(
                                              'id',r.id,
                                              'role', r.role
                                            )) AS roles
...
export const GET_USER_BY_EMAIL_WITH_ROLES = `SELECT u.id, u.name, u.email, u.image, u.active, u.password, ARRAY_AGG(
                                              json_build_object(
                                                'id',r.id,
                                                'role', r.role
                                              )) AS roles
...
export const GET_USERS_AND_ROLES_PAGINATE = `SELECT u.id, u.name, u.email, u.image, u.active, ARRAY_AGG(
                                              json_build_object(
                                                'id',r.id,
                                                'role', r.role
                                              )) AS roles
