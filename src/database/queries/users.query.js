...
SELECT u.id, u.name, u.email, u.image, u.active, ARRAY_AGG(
  json_build_object(
    'id',r.id,
    'role', r.role
  )) AS roles
...
SELECT u.id, u.name, u.email, u.image, u.active, u.password, ARRAY_AGG(
  json_build_object(
    'id',r.id,
    'role', r.role
  )) AS roles
...
SELECT ARRAY_AGG(id) AS rolesid FROM roles
`;

export const GET_USERS_PAGINATE = `
SELECT id, name, email FROM users OFFSET $1 LIMIT $2
`;
